...
config/
  ...
  modules/
    syncthing.nix                # Syncthing firewall ports (daw and fwork); device IDs documented here
...
Syncthing runs on `daw` and `fwork` as a **user-level** service for `fprice`, managed via `services.syncthing` in `config/home-manager/home.nix`. It starts after the rclone Dropbox FUSE mount and networking are ready.

Devices and folders are declared in Nix — `syncthing-init.service` (generated by home-manager) applies them via the API on each rebuild. To add or remove a device or folder, edit `home.nix` and run `rebuild`.

Firewall ports opened automatically (via `config/modules/syncthing.nix`):
- TCP/UDP 22000 — sync traffic
- UDP 21027 — local peer discovery

Known device IDs are documented as comments in `config/modules/syncthing.nix`.

### Adding a new device

1. Get the new device's ID: **Actions → Show ID** in its web UI, or `syncthing cli config system status | grep myID`.
2. Add it to `services.syncthing.settings.devices` in `home.nix`.
