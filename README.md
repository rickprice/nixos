...
Syncthing runs on **fwork only** as a user-level service for `fprice`. (`daw` uses Dropbox for file sync and does not run Syncthing.)

Configuration lives in `config/home-manager/syncthing-fprice.nix`, which is imported only by fwork's modules in `flake.nix`. It starts after the rclone Dropbox FUSE mount and networking are ready.

Devices and folders are declared in Nix — `syncthing-init.service` (generated by home-manager) applies them via the API on each rebuild. To add or remove a device or folder, edit `syncthing-fprice.nix` and run `rebuild`.

Firewall ports opened automatically on fwork (via `config/modules/syncthing.nix`):
